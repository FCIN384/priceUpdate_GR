...
async function main() {
  try {
    ...
    const GR_LIST = await playerSearch([210], 0); // playerSearch(시즌넘버, 최소오버롤)
    let GR_RESULTS = await playerPriceValue(GR_LIST, [1, 2, 3, 4, 5, 6, 7, 8]); // playerPriceValue(데이터 , 강화등급)
    await saveToDB(GR_RESULTS);
